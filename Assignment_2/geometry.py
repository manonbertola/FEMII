...
def getCantilever(lx=1000, ly=100, nx=50, ny=2):
    ...
    counter = 0
    connectivity = np.zeros((nx*ny, 4))

    for j in range(ny):
        for i in range(nx):
            connectivity[counter, :] = np.array([j*(nx+1)+i, j*(nx+1)+(i+1), (j+1)*(nx+1)+(i+1), (j+1)*(nx+1)+i])
            counter = counter + 1
    ...
    select_indexes = [0,1,2,4]
    dofs = dofs[select_indexes]
    ...
    index2 = set(np.where(coordinates[:, 1] > 0.499*ly)[0])
    index3 = set(np.where(coordinates[:, 1] < 0.510*ly)[0])
    index = list(index1.intersection(index2.intersection(index3)))

    BCs['natural'] = np.array([
        [int(2*index[0]+1), int(P)]
        ])
