...
def getBar():
    ...
    BCs = {}
    BCs["essential"] = np.array([[0, 0], [1, 0], [4, 0], [5, 0]])

    P = 1
    BCs["natural"] = np.array([[2, P]])
...
def getTruss():
    ...
    for j in range(ny):
        for i in range(nx):
            connectivity[counter, :] = np.array(
                [i + j * (nx + 1), i + 1 + j * (nx + 1)]
            )
            connectivity[counter + 1, :] = np.array(
                [i + 1 + j * (nx + 1), i + 1 + (j + 1) * (nx + 1)]
            )
            connectivity[counter + 2, :] = np.array(
                [i + j * (nx + 1), i + 1 + (j + 1) * (nx + 1)]
            )
            counter = counter + 3
    ...
    P = 1
    index = set(np.where(coordinates[:, 0] > 0.999 * lx)[0])
    index = np.array(list(index))

    BCs["natural"] = np.zeros((len(index), 2), dtype=int)
    BCs["natural"][:, 0] = 2 * index + 1
    BCs["natural"][:, 1] = P
