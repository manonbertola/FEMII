...
class Material:
    ...
    def setHardeningMode(self, hardeningMode):
        self.hardeningMode = False
        if hardeningMode == 'StrainHardening':
            self.hardeningMode = 'StrainHardening'
        elif hardeningMode == 'WorkHardening':
            self.hardeningMode = 'WorkHardening'
    ...
    def getStressAndTangent(self, stress, kappa, depsilon):
        ...
        p = 1
        #Definr Q matrix for strain hardening
        Q = np.array([[ 2/3, -1/3,   0],
                      [-1/3,  2/3,   0],
                      [   0,    0, 1/2]])
        ...
        if yfunction <= tolerance_yield*self.yield_stress: #Evaluate if yielding has occur
            ...
            while np.linalg.norm(residuals) > tolerance_residual*self.yield_stress and counter < maxit:
                ...
                if self.hardeningMode == 'StrainHardening':
                    kappa_dot_strain = deltaLambda_upd*np.sqrt((2/3)* m.T.dot(Q.dot(m)))
                    kappa_upd = kappa_upd + kappa_dot_strain

                elif self.hardeningMode == 'WorkHardening':
                    kappa_dot_work = deltaLambda_upd*stress.T.dot(m)
                    kappa_upd = kappa_upd + kappa_dot_work
